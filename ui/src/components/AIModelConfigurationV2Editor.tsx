...
function getByokInitialConfig(
    ...
    const matchesPipeline = (config: Record<string, unknown> | null) =>
        config ? !config.is_realtime : false;

    const byokConfiguration = byokConfigToLegacyShape(configuration);
    if (byokConfiguration) {
        return matchesPipeline(byokConfiguration) ? byokConfiguration : emptyByokInitialConfig();
    }
    ...
    return matchesPipeline(effective) ? (effective as Record<string, unknown>) : emptyByokInitialConfig();
}
...
export function AIModelConfigurationV2Editor({
    ...
    useEffect(() => {
        ...
        setPipelineInitialConfig(getByokInitialConfig(rawConfiguration, rawEffectiveConfiguration));
