...
export function AppSidebar() {
  ...
  const SidebarLink = ({ item }: { item: SidebarNavItem }) => {
    ...
    return (
      <SidebarMenuButton
        ...
        tooltip={tooltip}
        className={cn(
          "rounded-xl transition-colors hover:bg-sidebar-accent hover:text-sidebar-accent-foreground",
          isItemActive &&
            "bg-cta/20 font-semibold text-sidebar-foreground hover:bg-cta/25 hover:text-sidebar-foreground"
        )}
      >
        <Link
          href={item.url}
  ...
  return (
    <Sidebar collapsible="icon" variant="sidebar" className="app-sidebar-fixed">
      <SidebarHeader className="px-2 py-3 notranslate" translate="no">
        <div className="flex items-center justify-between">
          <div className={cn("flex items-center gap-2", isCollapsed && "hidden")}>
            ...
            >
              {isCollapsed ? (
                <BrandLogo mark inverse className="text-sm" />
              ) : (
                <BrandLogo inverse className="text-sm" />
              )}
            </Link>
          </div>
        ...
        {NAV_SECTIONS.map((section, index) => (
          ...
          >
            {section.label && (
              <SidebarGroupLabel
                className={cn(
                  "notranslate text-xs font-semibold uppercase tracking-wider text-sidebar-foreground/60",
                  isCollapsed && "hidden"
                )}
                translate="no"
              >
      ...
      >
        <div className="space-y-2">
          ...
          {provider === "stack" && (
            <div
              className={cn(
                "flex items-center justify-center rounded-full border border-border/60 bg-muted/30 p-1",
                isCollapsed && "flex-col"
              )}
            >
              <DropdownMenu>
